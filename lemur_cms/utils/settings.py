...
from lemur_cms.conf.spec import CONF_SPEC
from lemur_cms.conf.base import ModuleConfig

BLACKLIST = []
LEMURCMS_MODULES = None
...
def _get_key_from_module(mod, key, default):
    if hasattr(mod, key):
        ...
        value = getattr(mod, 'LEMURCMS_%s' % key.upper(), default)
    return value
...
def _is_lemur_cms_module(whatever):
    '''check if is lemur_cms module'''

    # check if is python module
    if hasattr(whatever, 'default') \
            or hasattr(whatever, 'lemur_cms_module_conf'):
        return True
...
def _get_correct_module(mod):
    ...
    check if is ``lemur_cms_module_conf`` specified and then import them
    """

    module_location = getattr(
        mod, 'lemur_cms_module_conf',
        getattr(mod, "LEMURCMS_MODULE_CONF", None))
    if module_location:
        ...
        # check if is lemur_cms config compliant
        if _is_lemur_cms_module(config_class):
            mod = config_class
