...
def get_all_widget_classes():
    """returns collected LemurCMS Widgets
...
def render_region(widget=None, request=None, view=None,
    ...
    lemur_cms_page = widget.parent if widget else page
    render_region = widget.region if widget else region

    # call processors
    for fn in reversed(list(lemur_cms_page.request_processors.values())):
        try:
            r = fn(lemur_cms_page, request)
        except:
            pass

    contents = {}

    for content in lemur_cms_page.content.all_of_type(tuple(
            lemur_cms_page._feincms_content_types_with_process)):
    ...
    from lemur_cms.templatetags.lemur_cms_tags import _render_content

    region_content = ''.join(
        contents[content.fe_identifier] if content.fe_identifier in contents else _render_content(
            content, request=request, context={})
        for content in getattr(lemur_cms_page.content, render_region))
